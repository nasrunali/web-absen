...
if (isset($_GET['kelas'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM tb_siswa WHERE kelas = ?");
                        ...
                        <table class="table text-start align-middle table-bordered table-hover mb-0">
                            <div class="form-group">
                                ...
                                <select class="form-select" id="kelas">
                                    <option value="satu">Kelas 1</option>
                                    <option value="dua">Kelas 2</option>
                                    <option value="tiga">Kelas 3</option>
                                </select>
                            </div>
                            <thead>
                                <tr class="text-white">
                            ...
                            <tbody>
                                ...
                                // SQL query to retrieve data from tb_siswa table
...
function updateTable() {
    ...
    // Gunakan jQuery untuk mengambil data dari endpoint PHP berdasarkan kelas yang dipilih
    $.get("getData.php", { kelas: selectedKelas }, function(data) {
        var table = document.getElementById("dataSiswa");
        table.innerHTML = "<thead><tr><th>Nama</th><th>NIS</th></tr></thead><tbody>";

        // Parse data JSON yang diterima dari PHP
        var siswaData = JSON.parse(data);

        // Isi tabel dengan data yang diambil
        siswaData.forEach(function(siswa) {
            var row = "<tr><td>" + siswa.nama + "</td><td>" + siswa.nis + "</td></tr>";
            table.innerHTML += row;
        });
...
// Panggil fungsi updateTable saat pemilihan kelas (Kelas) berubah
document.getElementById("kelas").addEventListener("change", updateTable);

// Panggil fungsi updateTable saat halaman dimuat untuk mengisi tabel dengan data default
